...
class Tutor(models.Model):
    ...
    estudiantes_ids = fields.Many2many('school.estudiante', string='Estudiantes a Cargo')
    ...
    def create(self, vals):
        ...
        if 'user_id' not in vals:
            tutor_name = vals.get('name', '').strip()  # Obtenemos el nombre y eliminamos espacios al inicio y final
            login = tutor_name.lower().replace(' ', '.')
