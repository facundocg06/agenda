...
class AuthController(http.Controller):
    ...
    def login(self, **kwargs):
        """
        Endpoint para iniciar sesión y devolver un token de sesión.
        :param kwargs: Parámetros JSON: 'email' y 'password'.
        :return: JSON con el token de sesión o un mensaje de error.
        """
        ...
        try:
            ...
            # Construir la respuesta con información del usuario
            return {
                'status': 'success',
...
# #         """ Endpoint para autenticar a un usuario y devolver un token de sesión si es válido. """
# #         params = json.loads(request.httprequest.data)
# #         print(params)
# #         username = params.get('username')
# #         password = params.get('password')
...
# #             return {'error': 'Faltan datos de autenticación'}

# #         user = request.env['res.users'].sudo().search([('login', '=', username)], limit=1)

# #         if user:
# #             # Verificamos si la contraseña es correcta
# #             if user._check_credentials(password, user_agent_env=None):
